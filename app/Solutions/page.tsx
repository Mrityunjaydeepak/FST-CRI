              ...
              <div className="space-y-4 pr-2 relative">
                {cardContents.map((card, index) => (
                  <div key={index} className="w-full">
                    {/* Card Component (You can replace this with your custom Card Component) */}
                    <div
  key={index}
  onClick={() => setSelectedCardIndex(index === selectedCardIndex ? null : index)}
  className={`relative bg-[#2d2d2d] flex items-center justify-center rounded-lg h-24 sm:h-28 cursor-pointer p-4 transition-all ${
    selectedCardIndex === index ? "border-2 border-white" : ""
  }`}
>
  {/* Dynamic Arrow based on Screen Size */}
  ...
  <h1 className="text-white text-center flex flex-row justify-center text-xl sm:text-2xl font-bold">
    {card.title}
                    ...
                    {/* Mobile View: Show Content Below Selected Card */}
                    {isMobile && selectedCardIndex === index && (
                      <div className="mt-4 p-4 bg-[#1a1a1a] rounded-lg">
                        ...
                        <ul className="list-disc list-inside text-white text-lg leading-relaxed space-y-2 mt-1">
                          {cardContents[selectedCardIndex].content.processList.map((item, i) => (
                            <li key={i}>{item}</li>
                          ))}
                        </ul>
                      </div>
          ...
          {/* Desktop View: Show Content in Separate Section */}
          {!isMobile && selectedCardIndex !== null && (
